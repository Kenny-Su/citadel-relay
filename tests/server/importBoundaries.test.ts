...
describe('app package import boundaries', () => {
  ...
  it('keeps bundled app assembly on public app package surfaces', () => {
    ...
    for (const packageName of bundledApps.packages) {
      ...
      expect(appPackageJson.scripts?.prebuild).toBe('node ../../../scripts/generate-local-app-metadata.mjs --package-dir .');
      expect(appPackageJson.scripts?.['prebuild:watch']).toBe('node ../../../scripts/generate-local-app-metadata.mjs --package-dir .');
      expect(appPackageJson.scripts?.pretypecheck).toBe('node ../../../scripts/generate-local-app-metadata.mjs --package-dir .');
