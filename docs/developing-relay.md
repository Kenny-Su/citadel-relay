...
Keep client token issuance, ACLs, subrooms, presence, app validation, persistence, and domain behavior in namespace owners. The relay verifies global client JWT identity and forwards only the trusted subject; identity must never imply admission or authorization.
